## Requirements

[Flask 2.0.1](https://pypi.org/project/Flask/)
- [Flask-SQLAlchemy 2.5.1](https://pypi.org/project/Flask-SQLAlchemy/)

[SQLAlchemy 1.4.17](https://pypi.org/project/SQLAlchemy/)

[gunicorn 20.1.0](https://pypi.org/project/gunicorn/)

[typing-extensions 3.10.0.0](https://pypi.org/project/typing-extensions/)

Python Packages required:
- [click 8.0.1](https://pypi.org/project/click/)
- [colorama 0.4.4](https://pypi.org/project/colorama/)
- [greenlet 1.1.0](https://pypi.org/project/greenlet/)
- [importlib-metadata 4.3.0](https://pypi.org/project/importlib-metadata/)
- [itsdangerous 2.0.1](https://pypi.org/project/itsdangerous/)
- [Jinja2 3.0.1](https://pypi.org/project/Jinja2/)
- [MarkupSafe 2.0.1](https://pypi.org/project/MarkupSafe/)
- [Werkzeug 2.0.1](https://pypi.org/project/Werkzeug/)
- [zipp 3.4.1](https://pypi.org/project/zipp/)


